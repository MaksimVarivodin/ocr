...
_DEFAULT_PROMPTS = {
    ...
    "batch_translate": (
        "You are a professional translator. Translate the following array of items into {target_language}. "
...
def translate_text_openrouter(
    ...
    - @param target_language: Target language (Russian names supported).
    ...
    prompt = _fill_prompt_template(tmpl, target_language=target_language, source_text=text)
    try:
...
def _extract_batch_translations(response_text: str) -> List[Tuple[int, str]]:
    ...
    try:
        ...
        return []
    items_keys = ["items", "элементы", "ответ", "result", "results"]
    items = None
    for k in items_keys:
    ...
    for it in items:
        if not isinstance(it, dict):
            continue
        idx = it.get("id") if isinstance(it.get("id"), int) else it.get("индекс")
        if not isinstance(idx, int):
            continue
        txt = it.get("translated") or it.get("перевод") or it.get("text") or it.get("текст")
...
def translate_blocks_openrouter(
    ...
    """Translate a list of structured OCR blocks (one string per block).
    ...
    - @param target_language: Target language.
    - @param batch_size: Number of blocks per request.
    ...
    while i < len(texts):
        ...
        out_chunk = translate_batch_openrouter(client, model, chunk, target_language, timeout)
        for j, t in enumerate(out_chunk):
            if on_paragraph:
                try:
                    on_paragraph(i + j, t)
                except Exception:
